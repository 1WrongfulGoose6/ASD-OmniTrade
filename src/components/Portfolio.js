  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-gradient-to-br from-blue-600 to-blue-400 text-white">
      ...
      <section className="relative z-10 mx-auto max-w-6xl px-8 pt-12">
        ...
        <p className="mt-2 text-blue-100/90">Track your holdings and performance over time.</p>
      </section>
          ...
          <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 text-center">
              <div>
                ...
                <p className="mt-1 text-3xl font-bold">${totalValue.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
              </div>
              <div>
                <p className="text-xs uppercase tracking-wide text-gray-500">P/L (Today)</p>
                <p className={`mt-1 text-3xl font-bold ${totalProfitLoss >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>${totalProfitLoss.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
              </div>
              <div>
                <p className="text-xs uppercase tracking-wide text-gray-500">Performance ({range})</p>
                <p className={`mt-1 text-3xl font-bold ${rangePerf[range] >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>{rangePerf[range].toFixed(2)}%</p>
              </div>
            </div>
            <div className="flex flex-wrap items-center gap-2 md:justify-end">
              {['24h', '7d', '1m', 'YTD', '1y'].map(r => (
                <button key={r} onClick={() => setRange(r)} className={`rounded-full px-3 py-1 text-sm font-medium transition ${range === r ? 'bg-blue-700 text-white' : 'bg-blue-100 text-blue-700 hover:bg-blue-200'}`}>
                  {r}
                </button>
              ))}
            </div>
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody>
                {holdings.map((holding) => (
                  <tr key={holding.symbol} className="border-b border-gray-200/80 last:border-0">
                    <td className="p-3 font-semibold text-gray-900">{holding.symbol}</td>
                    <td className="p-3 text-gray-700">${holding.price.toFixed(2)}</td>
                    <td className={`p-3 ${holding.change >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
                      {holding.change.toFixed(2)} ({holding.changePercent.toFixed(2)}%)
                    </td>
                    <td className="p-3 hidden md:table-cell text-gray-700">{holding.shares}</td>
                    <td className="p-3 hidden md:table-cell text-gray-700">${holding.value.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                    <td className={`p-3 ${holding.profitLoss >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>${holding.profitLoss.toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
